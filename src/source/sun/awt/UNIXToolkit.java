...
import sun.security.action.GetIntegerAction;
   ...
   1    *
   1    * This code is distributed in the hope that it will be useful, but WITHOUT
   1    * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
   1    * FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License
   1    * version 2 for more details (a copy is included in the LICENSE file that
   1    * accompanied this code).
   1    *
   1    * You should have received a copy of the GNU General Public License version
   1    * 2 along with this work; if not, write to the Free Software Foundation,
   1    * Inc., 51 Franklin St, Fifth Floor, Boston, MA 02110-1301 USA.
   2    *
   2    * Please contact Oracle, 500 Oracle Parkway, Redwood Shores, CA 94065 USA
   2    * or visit www.oracle.com if you need additional information or have any
   2    * questions.
   2    */
...
public abstract class UNIXToolkit extends SunToolkit {
  ...
  /**
   * Returns true if the native GTK libraries are capable of being loaded and are expected to work properly, false
   * otherwise.  Note that this method will not leave the native GTK libraries loaded if they haven't already been
   * loaded.  This allows, for example, Swing's GTK L&F to test for the presence of native GTK support without leaving
   * the native libraries loaded.  To attempt long-term loading of the native GTK libraries, use the loadGTK() method
   * instead.
   */
  @Override
  ...
  /**
   * Loads the GTK libraries, if necessary.  The first time this method is called, it will attempt to load the native
   * GTK library.  If successful, it leaves the library open and returns true; otherwise, the library is left closed and
   * returns false.  On future calls to this method, the status of the first attempt is returned (a simple lightweight
   ...
   *
   * @param longname a desktop property name. This contains icon name, size and orientation, e.g.
   * <code>"gtk.icon.gtk-add.4.rtl"</code>
   * @return an <code>Image</code> for the icon, or <code>null</code> if the icon could not be loaded
   */
  ...
  /**
   * Returns a BufferedImage which contains the Gtk icon requested.  If no such icon exists or an error occurs loading
   * the icon the result will be null.
   *
  ...
  /**
   * Returns a BufferedImage which contains the Gtk stock icon requested. If no such stock icon exists the result will
   * be null.
   *
   * @param widgetType one of WidgetType values defined in GTKNativeEngine or -1 for system default stock icon.
   * @param stockId String which defines the stock id of the gtk item. For a complete list reference the API at
   * www.gtk.org for StockItems.
   * @param iconSize One of the GtkIconSize values defined in GTKConstants
//   * @param textDirection One of the TextDirection values defined in GTKConstants
   * @param detail Render detail that is passed to the native engine (feel free to pass null)
  ...
  /**
   * This method is used by JNI as a callback from load_stock_icon. Image data is passed back to us via this method and
   * loaded into the local BufferedImage and then returned via getStockIcon.
   *
  ...
  protected RenderingHints getDesktopAAHints() {
    ...
    if (aaValue == null) {
      ...
      if (aaValue != null) {
        return new RenderingHints(new Key(1,""), aaValue);
      } else {
    ...
    return new RenderingHints(new Key(1,""), "");
  }
   ...
   * @return {@code true} if the GTK+ library is compatible with the given version.
   */
